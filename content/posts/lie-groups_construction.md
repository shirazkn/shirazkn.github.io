...
The <span class=accented>outer semi-direct product</span> follows by observing that the preceding definition can be generalized to the case where $H$ and $N$ are not subgroups of some common group. Moreover, the resulting product $G$ is not given to us to begin with. We are instead given (in lieu of $G$) an action $\Phi_h(\cdot):N \rightarrow N$ for each element $h\in H$, such that $\Phi_h(\cdot)$ is a group automorphism of $N$. Observe that if $H,N \leq G$ and $N\trianglelefteq G$ as before, then the choice of $\Phi_h(n) \coloneqq  h n h^{-1}$ will make the outer semi-direct product coincide with the inner semi-direct product in $G$. In the more general setting where $H$ and $N$ are <i>only</i> related to each other via actions of the form $\Phi_h(n)$, the group operation $\star$ is given by
